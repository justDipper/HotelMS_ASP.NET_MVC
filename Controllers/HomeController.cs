...
namespace HotelMS_ASP.NET_MVC.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger)
        {
            ...
            {
                BaseAddress = new Uri("https://localhost:7102/Login/Request")
            };
        ...
        [HttpPost]
        public async Task<IActionResult> Index(LoginAccessModel model) // Use the correct model type
        {
            //declare the Variable that get from the MODEL
            string user = model.username;
            string password = model.password;
            //Serialize the Model
            var jsonBody = JsonConvert.SerializeObject(new { username = user, password });

            try // Handle potential exceptions during HTTP request
            {
                //This will make a respost to WEB API
                var response = await _httpClient.PostAsync("https://localhost:7102/Login/Request", new StringContent(jsonBody, Encoding.UTF8, "application/json"));
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (jsonObject.authorization == "admin")
                    {
                        ...
                        HttpContext.Session.SetString("customerid", $"{jsonObject.customerid}");
